...
for comp in "${COMPONENTS[@]}"; do
    ...
    if [  -z "${COMPONENT}" -o \( "${COMPONENT}" == "${comp_lower}" \) ]; then
        ...
        if [ $rv -ne 0 ]; then
            ...
            echo "  is not idempotent. Reset the configuration of the component e.g. 'git reset --hard' (note: you will"
            echo "  loose all changes in the repository!) before repeating the deploy of the component."
